...
const commands = {
    identitysecret(data, acc) {
        if (!data) {
            console.log("Usando: identity_secret <base64 identity_secret for your account>");
            return;
        }

        acc.identity_secret = data;
        Config.saveAccount(acc);
        console.log("identity_secret guardado. Usando modo de confirmacion: " + automatic.confirmationsMode() + ". (help confirmations for info)");
        require('./confirmations').setSecret(data);
    },
    toggle(data) {
        let name = data.toLowerCase();
        if (configToggleNames.indexOf(name) === -1) {
            return console.log(`Config desconocida: ${name}. Lista: ${configToggles.join(", ")}`);
        }
...
function showCommands() {
    console.log("Comandos: " + Object.keys(commands).filter(command => command[0] !== '.').join(", "));
    console.log("Usar help [command name] para ver ayuda sobre el comando. Usar <TAB> para autocompletado.");
}
...
exports.startConsole = (Automatic) => {
    ...
    input.on('line', function(line) {
        ...
        if (commands[command]) {
            ...
            console.log("Comando desconocido: " + parts[0]);
            showCommands();
        }
    });
};