...
function heartbeat() {
    ...
    return new Promise((resolve, reject) => {
        Utils.postJSON({
            url: automatic.apiPath("automatic/v1/pulse"),
            form: params
        }).then(([body, c, status]) => {
            if (!body.hasOwnProperty("bumped")) {
                log.warn("Token invalido (backpack.tf): " + body.message || "(sin mensaje)");

                if (body.message.match(/does not exist/)) {
                    log.warn("backpack.tf api key registrada? Buscar en backpack.tf/settings > Advanced.");
                }
            ...
            if (bumped) {
                updates.push(`${bumped} listing${bumped === 1 ? '' : 's'} bumpeado(s).`);
            }
            if (currenciesChanged) {
                updates.push(`Precios actualizados.`);
            }
            ...
            log[updates.length ? "info" : "verbose"](`Checkeando backpack.tf... ${updates.join(" ")}`);
            resolve(1000 * 60 * 5);
        }).catch((msg, statusCode) => {
            if (Array.isArray(msg) && msg.length === 3) { // Cloudflare
                msg = msg[0] + "; backpack.tf puede estar caido, of bloqueado por Cloudflare."
            }

            log.warn(`Error ${statusCode || ""} occurred contacting backpack.tf (${msg}), volviendo a intentar en 60 segs`.trim());
            reject(1000 * 60 * 1);
        });
    });
}
...
function handleSellOrder(offer, listings) {
    ...
    let {
        metalOk: metalOk,
        keysOk: keysOk
    } = exchangeCurrencies(ours, theirs, {
...
function acceptOffer(offer) {
    ...
    let message = offer.summary({
        includeBuyOrders: true
    });

    offer.log("trade", "Accepting, summary:\r\n" + message);
    offerSummaries[offer.tid] = message;
    offer.accept().then((status) => {
        offer.log("trade", `aceptado${status === 'pending' ? "; confirmacion requerida" : ""}`);
    }).catch((msg) => {
        offer.log("warn", `no se puede aceptar: ${msg}`);
    });
}
...
function notifyOfferAccepted(offer) {
    ...
    if (!summary) {
        log.debug("oferta " + offer.id + " cancelada");
        return;
    }
    ...
    Utils.postForm({
            ...
            log.debug(`Notificacion enviada a backpack.tf por el trade #${offer.id}`);
            offerSummaries[offer.id] = null;
        })
        .catch((msg) => {
            if (Array.isArray(msg) && msg.length === 3) { // Cloudflare
                msg = msg[0] + "; backpack.tf puede estar caido, o bloqueado por Cloudflare."
            }

            log.warn("Problema contactando con backpack.tf (" + msg + "), reintentando en 1 min. (Trade completado)");
